...
void tk_add_test(struct tk_testcase t) {
    ...
    if (t.argv) {
        ...
        tk_assert(t.argc + 1 <= TK_MAX_ARGV_LEN,
                  "System test supports up to %d argv entries",
                  TK_MAX_ARGV_LEN - 1);
...
static bool check_results(struct tk_testcase *t, int status) {
    ...
    if (WIFEXITED(status)) {
        ...
        // System tests inspect main()'s exit status themselves; unit tests
        // should only pass on a clean zero exit.
        succ = t->stest || exit_status == 0;
        if (succ) {
            printf("- [%s] %s (%s)\n", pcol("PASS", 32), t->name, t->loc);
        } else {
            printf("- [%s] %s (%s) - %s (%d)\n",
                   pcol("FAIL", 31), t->name, t->loc,
                   pcol("Exited", 31), exit_status);
        }
    } else {
